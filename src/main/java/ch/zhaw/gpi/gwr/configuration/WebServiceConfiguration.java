package ch.zhaw.gpi.gwr.configuration;

import ch.zhaw.gpi.gwr.endpoint.GebaeudeUndWohnungsRegisterServiceEndpoint;
...
public class WebServiceConfiguration {    
     ...
     */
    @Bean
    public GebaeudeUndWohnungsRegisterServiceEndpoint gebaeudeUndWohnungsregisterService() {
        return new GebaeudeUndWohnungsRegisterServiceEndpoint();
    }

    /**
     * Definition der Endpunkt-Schnittstelle unter Verwendung vom CXF-Servlet,
 den CXF-Komponenten (springBus) sowie der Klasse, welche die Schnittstelle
 weiter definiert (GwrServiceEndpoint).
     * 
    ...
    public Endpoint jaxwsEndpoint() {
        ...
        // und der Schnittstellen-Definitions-Klasse wird der endpoint-Variable zugewiesen
        endpoint = new EndpointImpl(springBus(), gebaeudeUndWohnungsregisterService());
        // Der Endpoint wird publiziert, das heisst, ab diesem Zeitpunkt kann über
        // http://localhost:PORT/soap/GebaeudeUndWohnungsregisterService auf den Webservice
        // zugegriffen werden
        endpoint.publish("/GebaeudeUndWohnungsRegisterService");
